<x-layouts.app title="Allocate New Benefit - নতুন সুবিধা বরাদ্দ">
    <x-admin.shell>
        <div class="mx-auto max-w-4xl space-y-5">
            ...
            <div class="rounded-3xl bg-white p-6 sm:p-8 shadow-2xs border border-slate-200/80 text-slate-800">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-4 mb-6 gap-3">
                    <div>
                        <h1 class="text-lg sm:text-xl font-black text-slate-800 tracking-tight">নতুন সুবিধা বরাদ্দ ফরম (Allocate Benefit)</h1>
                        <p class="text-[11px] font-semibold text-slate-500 mt-0.5">জুলাই ২৪ যোদ্ধা ও নাগরিকদের জন্য সরকারি অনুদান ও সুবিধা প্রদান বিবরণী</p>
                    </div>
                    <div>
                ...
                <form action="{{ route('admin.benefits.store') }}" method="POST" class="space-y-6">
                    @csrf
                        ...
                        <div>
                            ...
                            <select name="person_id" id="person_id" class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">
                                ...
                                @foreach ($people as $person)
                                    <option value="{{ $person->id }}" {{ (old('person_id', $selectedPersonId) == $person->id) ? 'selected' : '' }}>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                            ...
                            <div>
                                ...
                                <select name="type_of_benefit" id="type_of_benefit" required onchange="updateCategories()" class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">
                                    <option value="আর্থিক সুবিধা (Financial Aid)" {{ old('type_of_benefit') == 'আর্থিক সুবিধা (Financial Aid)' ? 'selected' : '' }}>আর্থিক সুবিধা (Financial Aid)</option>
                                    <option value="চিকিৎসা সহায়তা (Medical Assistance)" {{ old('type_of_benefit') == 'চিকিৎসা সহায়তা (Medical Assistance)' ? 'selected' : '' }}>চিকিৎসা সহায়তা (Medical Assistance)</option>
                                    <option value="শিক্ষা বৃত্তি (Educational Scholarship)" {{ old('type_of_benefit') == 'শিক্ষা বৃত্তি (Educational Scholarship)' ? 'selected' : '' }}>শিক্ষা বৃত্তি (Educational Scholarship)</option>
                                    <option value="কর্মসংস্থান ও চাকরি (Employment Opportunity)" {{ old('type_of_benefit') == 'কর্মসংস্থান ও চাকরি (Employment Opportunity)' ? 'selected' : '' }}>কর্মসংস্থান ও চাকরি (Employment Opportunity)</option>
                                    <option value="আবাসন সুবিধা (Housing Support)" {{ old('type_of_benefit') == 'আবাসন সুবিধা (Housing Support)' ? 'selected' : '' }}>আবাসন সুবিধা (Housing Support)</option>
                                    <option value="পুনর্বাসন সহায়তা (Rehabilitation Support)" {{ old('type_of_benefit') == 'পুনর্বাসন সহায়তা (Rehabilitation Support)' ? 'selected' : '' }}>পুনর্বাসন সহায়তা (Rehabilitation Support)</option>
                                    <option value="অন্যান্য সুবিধা (Other Benefit)" {{ old('type_of_benefit') == 'অন্যান্য সুবিধা (Other Benefit)' ? 'selected' : '' }}>অন্যান্য সুবিধা (Other Benefit)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="category" id="category" required class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">
                                    <!-- Options generated dynamically by JS -->
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" required
                                   placeholder="যেমন: জুলাই অভ্যুত্থানে আহত বীরদের জন্য বিশেষ চিকিৎসা অনুদান"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                            ...
                            <div>
                                ...
                                <div class="flex rounded-xl border border-slate-300 bg-white overflow-hidden shadow-2xs focus-within:border-[#00551c] focus-within:ring-2 focus-within:ring-[#00551c]/20">
                                    ...
                                    <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', 0) }}"
                            ...
                            <div>
                                ...
                                <select name="payment_method" id="payment_method" class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">
                                    <option value="Bank Transfer (BEFTN)">ব্যাংক ট্রান্সফার (Bank Transfer - BEFTN)</option>
                                    <option value="Mobile Banking (bKash/Nagad)">মোবাইল ব্যাংকিং (bKash / Nagad / Rocket)</option>
                                    <option value="Account Payee Cheque">চেক (Account Payee Cheque)</option>
                                    <option value="Direct Cash / Voucher">নগদ প্রদান (Direct Cash)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="approval_date" id="approval_date" value="{{ old('approval_date', date('Y-m-d')) }}"
                                       class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                                <label class="flex items-center gap-2 cursor-pointer bg-white border border-slate-300 rounded-xl px-3 py-2.5 hover:border-emerald-500 transition">
                                    <input type="radio" name="status" value="Approved" {{ old('status', 'Approved') == 'Approved' ? 'checked' : '' }} class="w-4 h-4 text-[#00551c] focus:ring-[#00551c]">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer bg-white border border-slate-300 rounded-xl px-3 py-2.5 hover:border-blue-500 transition">
                                    <input type="radio" name="status" value="Paid" {{ old('status') == 'Paid' ? 'checked' : '' }} class="w-4 h-4 text-blue-600 focus:ring-blue-600">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer bg-white border border-slate-300 rounded-xl px-3 py-2.5 hover:border-amber-500 transition">
                                    <input type="radio" name="status" value="Pending" {{ old('status') == 'Pending' ? 'checked' : '' }} class="w-4 h-4 text-amber-600 focus:ring-amber-600">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer bg-white border border-slate-300 rounded-xl px-3 py-2.5 hover:border-red-500 transition">
                                    <input type="radio" name="status" value="Hold" {{ old('status') == 'Hold' ? 'checked' : '' }} class="w-4 h-4 text-red-600 focus:ring-red-600">
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50/50 p-5">
                        ...
                        <textarea name="remarks" id="remarks" rows="3" placeholder="সুবিধা প্রদান বা বরাদ্দ সংক্রান্ত কোনো বিশেষ শর্ত বা মন্তব্য থাকলে এখানে লিখুন..."
                                  class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:border-[#00551c] focus:outline-none focus:ring-2 focus:ring-[#00551c]/20 transition shadow-2xs">{{ old('remarks') }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                        ...
                        <button type="submit" class="px-6 py-2.5 rounded-xl bg-[#00551c] hover:bg-[#004416] text-white font-extrabold text-xs transition shadow-md flex items-center gap-1.5">
                            <span>✓</span>
                            <span>সুবিধা সংরক্ষণ করুন (Save Benefit)</span>
                        </button>
                    </div>
